...
public class HttpRequest
{
    ...
    /*
     * Constructors with Auth
     */

    public HttpRequest(String uri, Auth auth){
        this(_default_method, uri, auth);
    }

    public HttpRequest(String method, String uri, Auth auth){
        this(method, uri, null, null, auth);
    }

    public HttpRequest(String uri, RequestHeader[] headers, Auth auth){
        this(_default_method, uri, headers, null, auth);
    }

    public HttpRequest(String uri, RequestParameter[] params, Auth auth){
        this(_default_method, uri, null, params, auth);
    }

    public HttpRequest(String method, String uri, RequestHeader[] headers, Auth auth){
        this(method, uri, headers, null, auth);
    }

    public HttpRequest(String method, String uri, RequestParameter[] params, Auth auth){
        this(method, uri, null, params, auth);
    }

    public HttpRequest(String uri, RequestHeader[] headers, RequestParameter[] params, Auth auth){
        this(_default_method, uri, headers, params, auth);
    }

    public HttpRequest(String method, String uri, RequestHeader[] headers, RequestParameter[] params, Auth auth){
        _method = method;
        _uri = uri;
        mapParams(params);
        mapHeaders(headers);
    ...
    private void mapAuth(Auth auth){
        if(auth != null) {
            mapHeaders(auth.getHeaders());
            mapParams(auth.getParams());
        }
    }
